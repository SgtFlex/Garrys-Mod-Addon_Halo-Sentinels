...
function SWEP:CustomOnPrimaryAttackEffects()
	...
	HWBolt:SetPos(self:GetOwner():GetPos() + self:GetOwner():OBBCenter() + Vector(0, 0, 35) + self:GetOwner():GetForward()*15)
	HWBolt:Spawn()
		if (self:GetOwner():IsNPC()) then
	HWBolt:GetPhysicsObject():SetVelocity((self:GetOwner():GetAimVector()-Vector(0,0,0.08))*2000)
	else
		HWBolt:GetPhysicsObject():SetVelocity((self:GetOwner():GetAimVector())*2000)
	end
