...
ENT.lights = {
	...
	HeadLight = {Object = "", Attachment = "eye", Color = "0 155 255", Scale = .25, Offset = Vector(4, 0, -5)}
}
ENT.limbs = {
	head = 			{Removed = false, Health = 10, Lights = {"HeadLight"}, Hitgroup = 501, Bodygroups = {"head"}, Gibs = {"models/sentinels/sentinel_a_head.mdl", "models/gibs/metal_gib5.mdl", "models/gibs/metal_gib5.mdl"}},
	gun =			{Removed = false, Health = 10, Lights = {"GunLight"}, Hitgroup = 504, Bodygroups = {"gun"}, Gibs = {"models/gibs/metal_gib5.mdl", "models/gibs/metal_gib5.mdl"}},
	left_arm = 		{Removed = false, Health = 7, Lights = {}, Hitgroup = 502, Bodygroups = {"left_arm"}, Gibs = {"models/sentinels/sentinel_a_arm.mdl", "models/gibs/metal_gib5.mdl", "models/gibs/metal_gib5.mdl"}},
	right_arm = 	{Removed = false, Health = 7, Lights = {}, Hitgroup = 503, Bodygroups = {"right_arm"}, Gibs = {"models/sentinels/sentinel_a_arm.mdl", "models/gibs/metal_gib5.mdl", "models/gibs/metal_gib5.mdl"}},
	left_leg = 		{Removed = false, Health = 7, Lights = {"LeftLight"}, Hitgroup = 505, Bodygroups = {"left_leg"}, Gibs = {"models/sentinels/sentinel_a_arm.mdl", "models/gibs/metal_gib5.mdl", "models/gibs/metal_gib5.mdl"}},
	right_leg = 	{Removed = false, Health = 7, Lights = {"RightLight"}, Hitgroup = 506, Bodygroups = {"right_leg"}, Gibs = {"models/sentinels/sentinel_a_arm.mdl", "models/gibs/metal_gib5.mdl", "models/gibs/metal_gib5.mdl"}},
}
ENT.spriteTrail = {
	Object = "",
	startWidth = 50,
...
function ENT:CustomOnInitialize()
	...
	self.attackStats["damage"] = self.attackStats["damage"] * GetConVarNumber("vj_sent_sentinelDamageModifier")
	self:UseConvars()
	self:SpawnLights()
	self:TypeOfDeath()
	self:InitializeSound()
	self:SetColor(self.Colors["modelColor"])
	self.spriteTrail["Object"] = util.SpriteTrail(self, 0,self.Colors["trailColor"],true,self.spriteTrail["startWidth"],self.spriteTrail["endWidth"],self.spriteTrail["lifetime"],0.01,"trails/smoke")
end
...
function ENT:UseConvars()
	self.beamTickRate = GetConVar("vj_sent_beamTickRate"):GetFloat()
	self.TimeUntilRangeAttackProjectileRelease = self.beamTickRate -- How much time until the projectile code is ran?
	self.RangeAttackReps = (self.attackStats["beamDuration"]/self.beamTickRate)
...
function ENT:CustomRangeAttackCode()
	...
	self.Accuracy = self.Accuracy + (self.StartingAccuracyPos*-self.beamTickRate)
	...
	if (self.fireTrace.Entity) then
		self.fireTrace.Entity:TakeDamage((self.beamTickRate*self.attackStats["damage"]/self.attackStats["beamDuration"]),self,self)
	end
...
function ENT:DropExtraWeapons()
	if (GetConVarNumber("vj_sent_sentinelsDropWeapon")==0) then return false end
...
function ENT:CreateCustomCorpse()
	...
	if (GetConVarNumber("vj_sent_useParticles")==1) then 
		timer.Create("FireFallFXCorpse"..self.uniqueId, .15, 3000, function() 
			if (IsValid(corpse)) then 
				effect:SetOrigin(corpse:GetPos()) 
				util.Effect("fireFallFX", effect) 
			end 
		end) 
	end
	if (GetConVarNumber("vj_sent_useParticles")==1) then 
		timer.Create("corpseSparks"..self.uniqueId,2.5,16,function () 
			if (IsValid(corpse)) then 
				effect:SetOrigin(corpse:GetPos())  
				util.Effect("cball_explode",effect) 
				corpse:EmitSound(Sound("npc_vj_sent_sentinel/sentinel_welder_impact/weld13.wav")) 
			end 
		end) 
	end
	timer.Simple(2.5,function() 
		if (IsValid(corpse) && corpse.ignite) then 
			if (vFireExists) then
				corpse:Ignite() 
			else
				corpse:Ignite(10, 30) 
...
function ENT:SetUpGibesOnDeath(dmginfo,hitgroup) --Gib a bunch on death
	...
	if (self.gib) then 
		for k,v in pairs(self.gibTable) do
			...
			local effect=EffectData()
			if (GetConVarNumber("vj_sent_useParticles")==1) then timer.Create("FireFallFXSmall"..self.uniqueId..k, 0.075, 300, function() 
...
-- function ENT:AA_Wander(ShouldPlayAnim,NoFace)
-- 	local calmspeed = self.Aerial_FlyingSpeed_Calm
-- 	local ForceDown = ForceDown or false
-- 	if self.MovementType == VJ_MOVETYPE_AQUATIC then
-- 		if self:WaterLevel() < 3 then self:AA_Stop() ForceDown = true end
