...
function ENT:CustomOnInitialize() 
	self.RadiusDamage = 70 * GetConVarNumber("vj_sent_sentinelDamageModifier")-- How much damage should it do when it hits something
...
function ENT:CustomOnPhysicsCollide(data,phys) 
	local hitEnts = ents.FindInSphere(self:GetPos(), 50)
	self:SetMoveType(MOVETYPE_NONE)
	for k,v in pairs(hitEnts) do
		if (IsValid(self:GetOwner())) then
		v:TakeDamage(self:GetOwner().attackStats["damage"]*GetConVarNumber("vj_sent_sentinelDamageModifier"), self:GetOwner(), self:GetOwner())
	else
		v:TakeDamage(50*GetConVarNumber("vj_sent_sentinelDamageModifier"))
		end
	end
	timer.Create("DestroyTimer"..self:EntIndex(), 1, 1, function()
