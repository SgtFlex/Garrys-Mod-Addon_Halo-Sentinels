...
function ENT:CreateCustomCorpse()
	...
	if (GetConVarNumber("vj_sent_useParticles")==1) then timer.Create("FireFallFXCorpse"..self.uniqueId, .15, 3000, function() if (IsValid(corpse)) then effect:SetOrigin(corpse:GetPos()) util.Effect("fireFallFX", effect) end end) end
	if (GetConVarNumber("vj_sent_useParticles")==1) then timer.Create("corpseSparks"..self.uniqueId,2.5,16,function () if (IsValid(corpse)) then effect:SetOrigin(corpse:GetPos())  util.Effect("cball_explode",effect) end corpse:EmitSound(Sound("npc_vj_sent_sentinel/sentinel_welder_impact/weld13.wav")) end) end
