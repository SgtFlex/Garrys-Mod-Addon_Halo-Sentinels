...
ENT.MoveCollideType = MOVECOLLIDE_FLY_CUSTOM -- Move type | Some examples: MOVECOLLIDE_FLY_BOUNCE, MOVECOLLIDE_FLY_SLIDE
...
ENT.SoundTbl_Startup = {"npc_vj_sent_sentinel/sentinel_gun/in.wav"}
...
function ENT:CustomOnInitialize() 
	...
	-- light:SetKeyValue("rendercolor","0 255 255")
	-- light:SetKeyValue("scale","0.4")
	-- light:SetPos(self:GetPos())
	-- light:SetParent(self)
	-- light:Spawn()
	-- light:Activate()
	util.SpriteTrail(self, 0,Color(0,255,255,255),true,20,5,1,20,"trails/smoke")
end
---------------------------------------------------------------------------------------------------------------------------------------------
...
---------------------------------------------------------------------------------------------------------------------------------------------
ENT.expField = 0
ENT.freezeDuration = 6
ENT.remRange = {""}
ENT.remLeap = {""}
ENT.remMelee = {true}
ENT.remEnt = {""}
function ENT:CustomOnPhysicsCollide(data,phys) 
	if (self.expField==0) then
		self:SetMoveType(MOVETYPE_NONE)
		self.expField = ents.Create("prop_physics")
		self.expField:SetOwner(self)
		self.expField:SetMaterial("models/props_combine/portalball001_sheet")
		self:EmitSound("npc_vj_sent_sentinel/sent_slowField.wav")
		self.expField:SetPos(self:GetPos())
		self.expField:SetModel("models/hunter/misc/sphere375x375.mdl")
		self.expField:SetModelScale(3, 0)
		self.expField:DrawShadow(false)
		local tableEnts = ents.FindInSphere(self:GetPos(), 300)
		timer.Create("DestroyTimer"..self:EntIndex(), self.freezeDuration+0.25, 1, function()
			if (IsValid(self.expField)) then
			self.expField:Remove()
			end
		...
		for k,v in pairs(tableEnts) do
				if !((IsValid(v:GetOwner())) && (v:GetOwner():GetClass()=="obj_vj_sent_slowfield")) then
				if (v:GetMaterial()!="models/props_combine/tprings_globe") then local oldMaterial = v:GetMaterial() end
				v:AddEFlags(EFL_NO_THINK_FUNCTION)
				if (IsValid(v:GetPhysicsObject())) then
					v:GetPhysicsObject():EnableMotion(false)
					v:GetPhysicsObject():Sleep()
					v:AddEFlags(EFL_NO_PHYSCANNON_INTERACTION)
				end
				v:SetMaterial("models/props_combine/tprings_globe")
				v:TakeDamage(10*GetConVarNumber("vj_sent_sentinelDamageModifier"), self:GetOwner(), self:GetOwner())
				timer.Create("unfreeze"..v:GetCreationID(), self.freezeDuration+0.2, 1, function()
					if (IsValid(v) && IsValid(self)) then
						v:SetMaterial(oldMaterial)
						v:RemoveEFlags(EFL_NO_THINK_FUNCTION)
						if (IsValid(v:GetPhysicsObject())) then
							v:GetPhysicsObject():EnableMotion(true)
							v:RemoveEFlags(EFL_NO_PHYSCANNON_INTERACTION)
